    ...
    try {
        ...
        const [sessionRes, targetRes] = await Promise.all([
            query(`SELECT id, status, opened_at, closed_at, opened_by
                ...
                [req.params.id]),
            query(`SELECT id, faculty, year, level, major, program, student_status, international, campus FROM nbu_activity_targets WHERE activity_id = $1 ORDER BY id`,
                [req.params.id]),
        ]);
...
router.post('/', async (req, res) => {
    ...
    const { title, description, location, activity_type, start_datetime, end_datetime, max_participants, staff_ids, targets, academic_year, semester, flex_message_json } = req.body;
    ...
    try {
        const result = await transaction(async (client) => {
            const { rows } = await client.query(
                `INSERT INTO nbu_activities (title, description, location, activity_type, start_datetime, end_datetime, max_participants, created_by, academic_year, semester, flex_message_json)
...
router.put('/:id', async (req, res) => {
    if (!isAdmin(req.user.role)) return res.status(403).json({ success: false, message: 'ไม่มีสิทธิ์' });
    const { title, description, location, activity_type, start_datetime, end_datetime, max_participants, is_active, staff_ids, targets, academic_year, semester, flex_message_json } = req.body;
    try {
        const { rows } = await query(
